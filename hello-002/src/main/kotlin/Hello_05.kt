...
fun main() {
    ...
    /**
     * Random 으로 생성된 start1과 end1 변수값을 사용하여
     * 작은값~큰값 순으로 짝수의 합을 구하시오
     * 작은값과 큰값을 비교하여
     * 작은값이 start1에 담기도록 변수값을 swap 하기?
     */
    for(num in 0..7){
        // 정상코드 작동
    }
    ...
    // start1과 end1 두 변수에 담긴값을 비교하여
    // 작은값이 start1에 담기고 큰값이 end1에 담기도록 처리

    // swap 코드 (정렬시 쓰이는 코드)
    // 만약 start1의 값이 크면 start1 과 end1을 서로 교환
    if(start1>end1){
        var temp=start1
        start1=end1
        end1=temp
...
println("$start1~$end1 범위의 짝수의합 : $even")
